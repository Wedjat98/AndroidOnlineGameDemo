...
$pas =='' && (die('请输入密码'));
$oid = createOrderNo();
$mysql = mysqli_connect('127.0.0.1','root','123456','yhlocal',3306) or die("数据库连接错误");
$mysql->query('set names utf8');
$xx = mysqli_fetch_assoc($mysql->query("SELECT uid,pass FROM cdk WHERE uid = '$uid' limit 1"));
$xx['uid'] == '' && (die('此ID未授权'));
$xx['pass'] != $pas && (die('后台密码错误'));
if($type == 1){
    $num == '' && ($num = '1'); 
